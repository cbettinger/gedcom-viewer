...
class Face:
    ...
    def getSimilaritiesTo(self, other):
        assert(type(self) is type(other) and self.characteristics.keys() == other.characteristics.keys())
        similarities = {}
        for c, o in self.characteristics.items():
            similarities.update({c: o.calculateSimilarityTo(other.characteristics.get(c))})
        return similarities